...
(типа как еще один ярлык на рабочем столе который ведет по тому же пути что и первый), те две этих перменных теперь будут 
ссылаться на один и теже еданные 
...
private: закрытый класс или член класса. Представляет полную противоположность модификатору public. Такой закрытый класс или член класса доступен 
только из кода в том же классе или контексте.

protected: такой член класса доступен из любого места в текущем классе или в производных классах. При этом производные классы могут располагаться
 в других сборках.

internal: класс и члены класса с подобным модификатором доступны из любого места кода в той же сборке, однако он недоступен для других программ 
и сборок (как в случае с модификатором public).

protected internal: совмещает функционал двух модификаторов. Классы и члены класса с таким модификатором доступны из текущей сборки
 и из производных классов.

private protected: такой член класса доступен из любого места в текущем классе или в производных классах, которые определены 
в той же сборке.
------------------------------------------
...
Мы часто хотим защитить свою программу от ошибок, связанных с null-строками. Это можно сделать
 ...
 или string.IsNullOrWhiteSpace(string)(возвращает trye если string пуста или string == null или string == "\t" или string == " " те строка ). 
...
------------------------------------------
 class Person
    ...
    class People
    {
        ...
        //индексатор
        public Person this[int index]// в отличии от свойств инджексатор не имеет названия вместо него идет ключевое слово this 
        {
            //объекты Person хранятся в классе в массиве data, для получения их по индексу в индексаторе определен блок get
            get
            {
                return data[index];
            }
...
При создании производного класса надо учитывать тип доступа к базовому классу - тип доступа к производному классу
 должен быть таким же, как и у базового класса, или более строгим. То есть, если базовый класс у нас имеет тип доступа internal,
...
который находится вверху иерархии классов и они типа таким образом обе эих перменных 
те перменная потомка и перменная родителя с присвоеной в нее переменноой потомка начинают указыать на один класс в куче , 
но перменной класса родителя будет доступна только та часть кода которая в ней собтвенно есть те 
дополнительные функции и поля из потомка ей недоступны
...
Но кроме восходящих преобразований от производного к базовому типу есть нисходящие преобразования или downcasting - 
от базового типа к производному.
Но автоматически такие преобразования не проходят, ведь не каждый человек (объект Person) является сотрудником предприятия 
(объектом Employee). И для нисходящего преобразования необходимо применить явное преобразования, указав в скобках тип, 
к которому нужно выполнить преобразование:
            ...
            Employee employee1 = (Employee)person;//после апкаста можно уже производить явное преобразование от Person к Employee
...
//поэтому мы можем преобразовать объект obj к любому типу который располагается в иерархии классов между типом object и Employee.

Если нам надо обратиться к каким-то отдельным свойствам илиз методам объекта, то нам необязательно присваивать преобразованный объект переменной :
...
public static implicit|explicit operator Тип_в_который_надо_преобразовать(исходный_тип param)// explicit если преобразование явное
...
public int Seconds { get; set; }
    ...
    public static explicit operator int(Counter counter)//преобразует объект Counter к типу int, то есть получает из Counter число.
    {
    ...
    {
        return new Counter { Seconds = x };// тут создается новый объект Counter, у которого устанавливается свойство Seconds.
    }
}
    ...
    static void Main(string[] args)
    {
        ...
        int x = (int)counter1;//преобразует объект Counter к типу int x, то есть получает из Counter число.explicit преобразование явное
    {
        Console.WriteLine(x);   

        Counter counter2 = x;//преобразует число x объект типа int к типу Counter. implicit  преобразование неявное
        Console.WriteLine(counter2.Seconds);  // 23
    }

если для преобразования потребуется более сложная логика, то просто так написать не получится. Например, во втором примере 
при преобразовании от типа Counter к типу Timer сложность преобразования возрастает, соотвественно лучше один раз определить оператор, 
чем по сто раз писать один и тот же набор операций. А в случае преобразований с более сложной логикой это тем более актуально
...
------------------------------------------
угу значит виратуальный метод или свойства могу находится в обычном классе родителе, могут иметь свою функциональность, при этом могут быть 
(а могут и не быть, тогда их функционал будет просто использован из класса родителя)  перпоределнными в потмках, вызыватся через экземпляр класса 
в котором они обьявлены.

простой пример это у класса Human есть метод Display выавода на экран поля Name , а в классе наслднике Employee этот метод перопределяется для вывода 
...
class Employee : Person
{
    ...
    //методы можно перопрделть а можно оставлять как есть в этом случае объекты Employee будут использовать реализацию метода Display 
    //из класса Person те из базового класса
    public override string Display();//А чтобы переопределить метод в классе-наследнике, этот метод определяется с модификатором override. 
...
class EmployeeTwo : Employee
{
    ...
    //то есть в любом производном классе, который не является прямым наследником от базового класса, можно переопределить виртуальные методы.
    //Например, мы можем определить класс Manager, который будет производным от Employee, и в нем также переопределить метод Display.
    {
...
Также можно запретить переопределение методов и свойств. В этом случае их надо объявлять с модификатором sealed
 ...
 public override sealed string Display();//производится только в произодном классе в класс родителе смысл делать нет тк проще убраьт virtual
...
{
    private decimal sum;
    public override decimal Sum//А чтобы переопределить свойство в классе-наследнике, этот свойство определяется с модификатором override. 
    //Переопределенный свойство в классе-наследнике должен иметь тот же набор параметров, что и виртуальный свойство в базовом классе.
    {
        get
        {
            return sum;
...
определение в классе-наследнике метода или свойства,которые соответствует по имени и набору параметров методу или свойству базового класса.
 ...
 class Person
    ...
    class Employee : Person
    {
        ...
        public new void Display() //этотт метод скрывает рализацию из класса Persone при этом неважно является ли метод в классе родителя виртуальным
                                  //или нет, если используется  var kek = (Employee) bob то мы получаем на выходе метод из Employee а не методж из класса Person
                                  //это полезно использовать когжа нпример метод Display в базовом классе не является виртуальным,
                                  //мы не можем его переопределить, и тут нам поможет именно такая конструкция
        {
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            //Потому, что объект создан и занимает в памяти 20 байт. И о не как не может кастнуться в объект который должен занимать 20 Мб.
            // переменной bob будет доступна только та часть, которая представляет функционал типа Person.
            // Это назыывается  восходящие преобразования
            ...
            kek.Display(); //Max Robertson работает в okla здесь используется сокрытый метод Display тк кек это обьект класса Employee 

            Employee tom = new Employee("Tom", "Smith", "Microsoft");//используется сокрытый метод Display тк кек это обьект класса Employee 
...
------------------------------------------
а вот абстрактные класс может иметь как обыкновенные переменные и методы и конструкторы (со стандартным наполнением и объявлением) так и абстрактные 
методы и переменные (но тогда у них не должно  быть тела функциональности, они должны быть обязательно переопределены в потомках,
 а так же класс в которм есть хоть один абстрактный член должен быть сам абстрактным), и вот  эти абстракты не могут быть вызваны в 
отличии от вируталов через экземпляры. 
...
и вот только когда мы преопределяем класс фигуры с полями типа Площадь и Периметр в какомнибудь там круге или треугольнике 
только там она начинает сущетсовать как бы
...
class Client : Person
{
        ...
        : base(name)
    {
     ...
     public override void DisplayIs()//переопреедлим асбтакртный класс из родителя, если этого не сделать компилятор выдаст ошибку 
        {
...
//И хотя в примере конструктор класса Person не вызывается, тем не менее производные классы Client и Employee могут обращаться к нему.
...
------------------------------------------
Xрестоматийным примером является система геометрических фигур. В реальности не существует геометрической фигуры как таковой.
 Есть круг, прямоугольник, квадрат, но просто фигуры нет. Однако же и круг, и прямоугольник имеют что-то общее и являются фигурами
...
Equals(gthjghtltkztncz) принимает в качестве параметра объект любого типа, который мы затем приводим к текущему, если они являются объектами одного класса.
 Затем сравниваем по именам. Если имена равны, возвращаем true, что будет говорить, что объекты равны. Однако при необходимости реализацию 
 метода можно сделать более сложной, например, сравнивать по нескольким свойствам при их наличии.
...
упаковка распкаовка
 ...
 class Account
    ...
    Account acc2 = new Account();
            ...
            //упкаовка и распкаовка ведут к снижению производительностьи и нарушению безопасности типа если обьекты изменяются извне
    //и мы можем не знать какие значения хранятся в этом обьекте если не те
 ...
 //чтобы решить проблему упаковки распкаовки и проблему безопасноти типов исопльзу.тся обощения
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Account<int> acc1 = new Account<int>();//тк класс теперь обобщенный нужно прри создани экземпляра куазать какой типа экземпляр будет исопльзовать
            Account<string> acc2 = new Account<string>();//тк класс теперь обобщенный нужно прри создани экземпляра куазать какой типа экземпляр будет исопльзовать
...
class Program
    {
        static void Main(string[] args)
        {
            ...
            Instantiator<Account<int>,string> i = new Instantiator<Account<int>, string>//кладев в обьект i наш класс Action с генериком int, а вторым генриком кладем 
            //простую строку
            {
                FromInstance = acc1,//кладем сюда экземпляр Acction<int>acc1 с вложеиями ввиде acc1.Id = 10 и acc1.Sum = 20
                ToInstance = acc2,//кладем сюда экземпляр Acction<int>acc2 с вложеиями ввиде acc1.Id = 30 и acc1.Sum = 60
...
Console.WriteLine($"{i.FromInstance.Id} {i.FromInstance.Sum} {i.ToInstance.Id} {i.ToInstance.Sum} { i.Code}");
            ...
            Instantiator<int, string> j = new Instantiator<int, string>//здесь просто тандартный рпимер с двумя генериками ввиде инт и стринг
            {
                FromInstance = 20,
                ToInstance = 10,
                Code = "2010"
...
так же можно использовать геенрик методы 
            ...
            int x = 10;
            int y = 30;
            Swap(ref x, ref y);//ref используется чтобы наш метод поменял значение непосрдсвтенно в перменны а не только внутри себя 
...
которые содержат два разных метода. И оба определенных здесь класса являются частичными. Для этого они определяются с ключевым словом partial.
==========================================
...
вызови вот эту вот вторую асинхронную функцию invike, но сделай это в основном потоке. Вторая функция обновляет индикатор процесса, 
и, поскольку пул потоков вызывает её в основном потоке, никаких конфликтов не происходит.
Типа вызываем например нашу  асинхронную функцию this в случае каогото дествия в программе, например нажатие кнопки (это делается как раз в асинхролнном потоке) 
и наша this делает скажем settext = "AOAOAAO" в textbox(это делается в оснвном потоке). а такак  settext = "AOAOAAO"в textbox делается в основном потоке, 
при налиции нескольких потоков(таких вот функций по типу this), напрмиер есть еще одна скаежм кнопка ктороая вызывает еще одну асинхронную функцию которая джаелет чтот подобное  settext = "blabla"в том же textbox по нажатию кнопки. И чтобы асинхронные функции для входа в оснвной поток друг друга не ждали и не конфликтовали. наши фунции асинхронная разбиваеттся на две  части первая делает полезную работу обработку события нажатия кнопки и говрит вызови вторую часть(которая собтсвенно делает gettext в textbox) в оснвном потоке да обновление происходит послдовательно но и  потому у нас не происходит конфликта с очередностью мы как раз и спользуем invoke ктороый явлется как бы окном в основной поток из асинхроного потка нашей функции this те invoke попросту используется для синхронного вызова каждого из первоч части нашей this ва которой выолппняется обработка нажатия, это означает, что вызывающий код(основной поток) должен ожидать завершения вызова через invoke, прежде чем продолжить свою работу
вобщем скажи мне правильно ли я поял прдназначение invoke

типа пул потков это как бы списко чтли потоков для выполнения в фновм режиме
типа чтобы выполниьт какойто метод из потока надо его в пул потока поместить и тогда пул сам будет извелкать методы и выполноять их при вызове
и вот чтобы эти асинхронные потоки не конфликтовали и существует invoke
