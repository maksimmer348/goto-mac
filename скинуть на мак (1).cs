...
НО если мы испобьзуем стандратныый тип исключений в купе с нетарндаркными типами нам необходим помещать 
  ...
  try
            {
                ...
                {
                    throw new Exception(
                        "длина строки больше 6 символов"); //этот оператор используется для генерации исключения,
                    //после этого оператора указывается обьект исключения например
                    //new Exception("сюда можно записать собобщение которое будет показываатся при исключении");
                    //а посколькку здесь указано исключение типа Exception то обрабатываться оно будет в соотвтетвующем блоке catch
                }
