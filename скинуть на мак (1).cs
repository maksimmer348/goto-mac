...
Task.Run() запускает задачу, а затем выполняет работу сразу после этой задачи, прежде чем задача завершится.
если в вызывающем задауче методе нет нужды ждать результаты задачи то исопльзуется это
...
(7)Введите число:
(5 и 5+)Факториал равен 720 (дожддались рун и выводим его по  готовности)
10
Квадрат числа равен 100

------------------------------------------
await Task.Run () запускает задачу, а затем выполняет другую работу, пока задача не будет завершена, после чего он выполняет работу после задачи.
